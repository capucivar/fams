                    ...
                    <label class="col-sm-3 control-label">代理商ID</label>
                     <div class="col-sm-8">
                        <div class="input-group"> 
                            <input id="AID" name="AID" type="text"  class="form-control" value="<?= !empty($lowerAgent)?$lowerAgent["AID"]:""?>">
...
<script type="text/javascript"> 
    ...
    function transferRCard() { 
        ...
        if($("#ANAME").text() == "" || $("#CELLPHONE").text() == "" || $("#WXID").text() == ""){
            baModalTipShow("提示", "抱歉，信息不完整", "d");
            $btn.html('转账');
            $btn.removeClass("disabled");
            return;
        }
...
//                    window.location = "/Agent/agentList";
    ...
    function getData(){
        var id = $("#AID").val(); 
        $.post("/Agent/getAgentById", {aid: $("#AID").val()}, function (response) { 
                if(response.result==""){
                    baModalTipShow("提示", "没有找到记录", "d");
                    return;
                }
